...
//   solids   lit meshes: pad, stage 1, interstage, stage 2, fairing halves
...
import { buildFalcon9, buildPad, F9_GEOM } from './rocket.js';
...
uniform float uBandY;     // soot band start, negative disables
out vec4 outColor;
void main() {
  ...
  vec3 base = uColor;
  // Scorch band on a flown booster. Purely cosmetic, but it is the detail
  // that makes a white cylinder read as a Falcon 9.
  if (uBandY > 0.0) {
    float band = smoothstep(uBandY + 4.5, uBandY, vWorld.y) * smoothstep(uBandY - 2.0, uBandY + 0.6, vWorld.y);
    base = mix(base, vec3(0.10, 0.10, 0.12), band * 0.85);
  }

  // Vertical seam shading so a smooth cylinder still reads as panelled metal.
  float seam = 0.94 + 0.06 * sin(atan(vWorld.z, vWorld.x) * 18.0);

  vec3 col = base * (uAmbient + diff * 0.95) * seam;
  float rim = pow(1.0 - max(dot(n, v), 0.0), 3.0);
  col += vec3(0.42, 0.58, 0.95) * rim * uRim;
  col += base * uEmissive;
...
export function createScene(canvas, { tier = 'high' } = {}) {
  ...
  const parts = buildFalcon9();
  const meshes = {
    stage1: uploadMesh(gl, parts.stage1),
    interstage: uploadMesh(gl, parts.interstage),
    stage2: uploadMesh(gl, parts.stage2),
    fairingA: uploadMesh(gl, parts.fairingA),
    fairingB: uploadMesh(gl, parts.fairingB),
  ...
  function resize() {
    ...
    if (state.holeMode) budget = Math.round(budget * 0.30);
  ...
  const drawMesh = (mesh, model, color, opts = {}) => {
    ...
    gl.uniform1f(uniforms.uBandY, opts.bandY ?? -1);
  ...
  function render(f, dt, time) {
    ...
    if (state.holeMode && dt > 0) {
      ...
      if (ms > 34 && state.steps > 24) state.steps -= Math.min(6, state.steps - 24);
    ...
    if (f.showVehicle || f.showEarth) {
      ...
      if (f.showEarth) {
        ...
        drawMesh(meshes.earth, m4.compose(f.earthPos, [0, time * 0.00002, 0], [s, s, s]),
          [0.10, 0.26, 0.52], { rim: 2.2, ambient: 0.3 });
      }

      if (f.showPad) {
        // Sunk below the engine bells so the plume fires above the deck
        // rather than through it, which is what a flame trench does.
        drawMesh(meshes.pad, m4.translation(0, -7, 0), [0.085, 0.092, 0.115], { rim: 0.18 });
      }

      if (f.showVehicle) {
        // Stage 1 keeps flying its own path after separation.
        if (f.stage1Visible) {
          const m1 = m4.compose(f.stage1Pos, f.stage1Rot, [1, 1, 1]);
          drawMesh(meshes.stage1, m1, [0.90, 0.91, 0.94], { bandY: 6, rim: 0.55 });
          drawMesh(meshes.interstage, m1, [0.86, 0.87, 0.90], { rim: 0.55 });
        }
        const m2 = m4.compose(f.stage2Pos, f.stage2Rot, [1, 1, 1]);
        drawMesh(meshes.stage2, m2, [0.90, 0.91, 0.94], { rim: 0.55 });
        if (f.fairingOn) {
          const spread = f.fairingSpread;
          const fa = m4.compose(
            [f.stage2Pos[0] - spread, f.stage2Pos[1], f.stage2Pos[2]],
            [f.stage2Rot[0], f.stage2Rot[1], f.stage2Rot[2] + spread * 0.06],
            [1, 1, 1]
          );
          const fb = m4.compose(
            [f.stage2Pos[0] + spread, f.stage2Pos[1], f.stage2Pos[2]],
          ...
          drawMesh(meshes.fairingA, fa, [0.93, 0.94, 0.96], { rim: 0.6 });
          drawMesh(meshes.fairingB, fb, [0.93, 0.94, 0.96], { rim: 0.6 });
        }
      }
    }
    ...
    if (f.thrust > 0.01 && f.showVehicle) {
      const origin = f.stage1Visible && f.thrustStage === 1
        ? [f.stage1Pos[0], f.stage1Pos[1] - 2.4, f.stage1Pos[2]]
        : [f.stage2Pos[0], f.stage2Pos[1] + F9_GEOM.interstageTop - 3.4, f.stage2Pos[2]];
      const rate = Math.min(60, Math.round(f.thrust * (f.thrustStage === 1 ? 34 : 12) * Math.min(dt, 0.05) * 60));
      emit(rate, origin, f.thrustStage === 1 ? 1.7 : 0.7, f.thrustStage === 1 ? 46 : 26, dt);
    }
    ...
    if (live > 0 && f.showVehicle) {
      ...
      gl.uniform1f(uniforms.uScale, f.thrustStage === 1 ? 1.5 : 0.85);
